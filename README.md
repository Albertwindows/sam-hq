...
最近的分割模型（SAM）代表了分割模型扩展的巨大飞跃，允许强大的零样本功能和灵活的提示。 尽管使用了11 亿个mask进行训练，但 SAM 的mask预测质量在许多情况下仍达不到要求，特别是在处理具有复杂结构的物体时。 我们提出了HQ-SAM，使 SAM 能够准确分割任何对象，同时保持 SAM 原有的快速设计、效率和零样本泛化性。 我们精心的设计重用并保留了 SAM 的预训练模型权重，同时仅引入最少的附加参数和计算。 我们设计了一个可学习的High-Quality Output Token，将其注入到 SAM 的掩模解码器中，负责预测高质量掩模。 我们不仅仅将其应用于掩模解码器特征，而是首先将它们与早期和最终的 ViT 特征融合，以改进掩模细节。 为了训练我们引入的可学习参数，我们构建了来自多个来源的 44K 细粒度掩模的数据集。 HQ-SAM 仅在引入的 44k 掩码 detaset 上进行训练，在 8 个 GPU 上仅需要 4 小时。 我们展示了 HQ-SAM 在跨不同下游任务的 9 个不同分割数据集中的功效，其中 7 个在零样本迁移策略中进行评估。
